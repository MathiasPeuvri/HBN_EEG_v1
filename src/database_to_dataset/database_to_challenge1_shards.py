...
def process_challenge_epochs(raw, events_df, verbose=False):
    """
    Process target events for challenge1 with pre-trial epochs.
    ...
    Returns:
        pd.DataFrame: DataFrame with pre-trial epochs and response metrics
    """
    if verbose:
        print("=== Challenge1 Event Processing ===")
    ...
    for idx, target_row in epochs_df.iterrows():
        ...
        epochs_df.at[idx, 'target_side'] = target_side
        epochs_df.at[idx, 'response_time'] = response_time
        epochs_df.at[idx, 'response_side'] = response_side
        epochs_df.at[idx, 'hit_accuracy'] = hit_accuracy
    
    if verbose and len(epochs_df) > 0:
        print(f"Created epochs DataFrame with shape: {epochs_df.shape}")
        print(f"Hit accuracy distribution: {epochs_df['hit_accuracy'].value_counts().to_dict()}")
        print(f"Mean response time: {epochs_df['response_time'].mean():.3f}s")
...
def create_challenge1_shards(
    ...
    tmin: float = -2.0,
    tmax: float = 0.0,
    target_events: List[str] = ['right_target', 'left_target'],
    verbose: bool = True
) -> None:
    """
    Create challenge1 data shards from EEG database.
    """
    # Make target_events available to process_challenge_epochs function
    global TARGET_EVENTS, TMIN, TMAX
    TARGET_EVENTS = target_events
    TMIN, TMAX = tmin, tmax
    
    if verbose:
        print('================ Start Challenge1 Sharding ========================')
    ...
    if verbose:
        print(f"Starting challenge1 data processing for {len(subjects_list)} subjects")
    ...
    for i, SUBJECT in enumerate(subjects_list):
        ...
        for RUN in runs:
            ...
            if not loader.data_exists(SUBJECT, task_name, run=RUN):
                print(f"{SUBJECT} {task_name} Run {RUN}: Data not found (if smart we would look first in participants.tsv to find the subjects that have the task data)")
                continue
            ...
            try:
                ...
                # Process events with challenge metrics
                epochs_df = process_challenge_epochs(raw_preprocessed, data['events'], verbose=False)
                ...
                if verbose and len(epochs_df) > 0:
                    print(f"Created {len(epochs_df)} challenge epochs for this run")
                    correct_count = (epochs_df['hit_accuracy'] == 'correct').sum()
                    print(f"Accuracy: {correct_count}/{len(epochs_df)} correct")
        ...
        if subject_epochs:
            ...
            challenge1_data.append(subject_df)
            subjects_processed += 1
            
            if verbose:
                print(f"Total epochs for {SUBJECT}: {len(subject_df)}")
                if len(subject_df) > 0:
                    accuracy_pct = (subject_df['hit_accuracy'] == 'correct').mean() * 100
        ...
        if (i + 1) % nb_subjects_per_shard == 0 or (i + 1) == len(subjects_list):
            if challenge1_data:
                ...
                shard_df = pd.concat(challenge1_data, ignore_index=True)
                
                # Save shard
                shard_path = savepath_root / f'challenge1_data_shard_{shard_counter}.pkl'
                ...
                if len(shard_df) > 0:
                    print(f"Hit accuracy distribution:")
                    print(shard_df['hit_accuracy'].value_counts())
                    print(f"Mean response time: {shard_df['response_time'].mean():.3f}s")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--tmin", type=float, default=-2.0, help="Start time for event epochs (seconds)")
    parser.add_argument("--tmax", type=float, default=0.0, help="End time for event epochs (seconds)")
    parser.add_argument("--target-events", nargs="+", default=['right_target', 'left_target'], help="Event types to extract (only targets for challenge)")
